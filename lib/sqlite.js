...
async function dbUpdateData(buyer, seller, value, unit, id, sellerIsAdmin){
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE Deals SET buyer = ?, seller = ?, value = ?, unit = ?, sellerIsAdmin = ? WHERE id = ?`, [buyer.toLowerCase(), seller.toLowerCase(), value, unit, sellerIsAdmin, id], function(err) {
        console.log(err)
    });
    console.log(isError);
    console.log("------------");
