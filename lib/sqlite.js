...
async function dbInsertData(buyer, seller, value, unit){
    ...
    if(check != null)
        return check;
    isError = await db.run(`INSERT INTO Deals(buyer, seller, value, unit) VALUES(?, ?, ?, ?)`, [buyer.toUpperCase(), seller.toUpperCase(), value, unit], function(err) {
        console.log(err)
    });
...
async function dbUpdateData(buyer, seller, value, unit, id, sellerIsAdmin){
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE Deals SET buyer = ?, seller = ?, value = ?, unit = ?, sellerIsAdmin = ? WHERE id = ?`, [buyer.toUpperCase(), seller.toUpperCase(), value, unit, id, sellerIsAdmin], function(err) {
        console.log(err)
    });
    if(isError.lastID == undefined)
        return createJsonAnswer(610, "Error: row hasn`t been inserted");
...
async function dbGetDealsByAccount(account){
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    dealsList = await db.all(`SELECT * FROM deals WHERE buyer = ? OR seller = ?`, [account.toUpperCase(), account.toUpperCase()], function(err, rows){
        console.log(err)
    });
