...
async function dbInsertData(buyer, seller, value, unit){
    ...
    if(check != null)
        return check;
    isError = await db.run(`INSERT INTO Deals(buyer, seller, value, unit) VALUES(?, ?, ?, ?)`, [buyer.toLowerCase(), seller.toLowerCase(), value, unit], function(err) {
        console.log(err)
    });
...
async function dbUpdateData(buyer, seller, value, unit, id, sellerIsAdmin){
    check = dbInsertDataCheck(buyer, seller, value, unit); // TODO: id check
    if(check != null)
        return check;
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE Deals SET buyer = ?, seller = ?, value = ?, unit = ?, sellerIsAdmin = ? WHERE id = ?`, [buyer.toLowerCase(), seller.toLowerCase(), value, unit, id, sellerIsAdmin], function(err) {
        console.log(err)
    });
    if(isError.lastID == undefined)
        return createJsonAnswer(610, "Error: row hasn`t been inserted");
