...
async function dbInsertData(buyer, seller, value, unit, sellerIsAdmin, fee, feeRole){
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    check = dbInsertDataCheck(buyer, seller, value, unit, sellerIsAdmin, fee, feeRole);
    if(check != null)
        return check;
    isError = await db.run(`INSERT INTO Deals(buyer, seller, value, unit, sellerIsAdmin, fee, feeRole) VALUES(?, ?, ?, ?, ?, ?, ?)`, [buyer.toLowerCase(), seller.toLowerCase(), value, unit, sellerIsAdmin, fee, feeRole], function(err) {
        console.log(err)
    });
...
async function dbUpdateData(buyer, seller, value, unit, id, sellerIsAdmin, fee, feeRole){
    check = dbInsertDataCheck(buyer, seller, value, unit, sellerIsAdmin, fee, feeRole); // TODO: id check
    if(check != null)
        return check;
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE Deals SET buyer = ?, seller = ?, value = ?, unit = ?, sellerIsAdmin = ?, fee = ?, feeRole = ? WHERE id = ?`, [buyer.toLowerCase(), seller.toLowerCase(), value, unit, sellerIsAdmin, fee, feeRole, id], function(err) {
        console.log(err)
    });
    if(isError.lastID == undefined)
        return createJsonAnswer(610, "Error: row hasn`t been inserted");
...
async function dbGetDealsByAccount(account){
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    dealsList = await db.all(`SELECT * FROM deals WHERE buyer = ? OR seller = ?`, [account.toLowerCase(), account.toLowerCase()], function(err, rows){
        console.log(err)
    });
...
async function setTxId(id, txId){
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE deals SET txId = ? WHERE id = ?`, [txId, id], function(err) {
        console.log(err)
    });
    if(dealsList.length == undefined){
        return createJsonAnswer(630, "Error: can`t get deals by id");}
