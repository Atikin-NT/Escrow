...
async function dbInsertData(buyer, seller, value, sellerIsAdmin, fee, feeRole){
    ...
    if(check != null)
        return check;
    isError = await db.run(`INSERT INTO Deals(buyer, seller, value, sellerIsAdmin, fee, feeRole) VALUES(?, ?, ?, ?, ?, ?)`, [buyer.toLowerCase(), seller.toLowerCase(), value, sellerIsAdmin, fee, feeRole], function(err) {
        console.log(err)
    });
...
async function dbUpdateData(buyer, seller, value, id, sellerIsAdmin, fee, feeRole){
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE Deals SET buyer = ?, seller = ?, value = ?, sellerIsAdmin = ?, fee = ?, feeRole = ? WHERE id = ?`, [buyer.toLowerCase(), seller.toLowerCase(), value, sellerIsAdmin, fee, feeRole, id], function(err) {
        console.log(err)
    });
    if(isError.lastID == undefined)
        return createJsonAnswer(610, "Error: row hasn`t been inserted");
...
async function dbUpdateDealStatus(txId, status) {
    ...
    if(db == null)
        return createJsonAnswer(600, "Connection error");
    isError = await db.run(`UPDATE deals SET status = ? WHERE txId = ?`, [status, txId], function(err) {
        console.log(err)
    });
    if(isError.lastID == undefined)
